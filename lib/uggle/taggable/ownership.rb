module Uggle::Taggable
  module Ownership
    def self.included(base)
      base.extend Uggle::Taggable::Ownership::ClassMethods

      base.class_eval do
        after_save :save_owned_tags
      end

      base.initialize_uggle_ownership
    end

    module ClassMethods
      def uggle(*args)
        initialize_uggle_ownership
        super(*args)
      end

      def initialize_uggle_ownership
    ...
    def owner_tags(owner)
      ...
      if self.class.preserve_tag_order?
        scope.order("#{Uggle::Tagging.table_name}.id")
      else
        scope
      end
    end
    ...
    def owner_tag_list_on(owner, context)
      ...
      cache[owner] ||= Uggle::TagList.new(*owner_tags_on(owner, context).map(&:name))
    end
    ...
    def set_owner_tag_list_on(owner, context, new_list)
      ...
      cache[owner] = Uggle.default_parser.new(new_list).parse
    end
    ...
    def save_owned_tags
      tagging_contexts.each do |context|
        cached_owned_tag_list_on(context).each do |owner, tag_list|
          ...
          Uggle::Tagging.where(taggable_id: id, taggable_type: self.class.base_class.to_s,
