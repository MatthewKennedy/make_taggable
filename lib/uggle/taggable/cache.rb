module Uggle::Taggable
  module Cache
    ...
    module Columns
      ...
      #   the call to :columns as @uggle_cache_columns
      ...
      def _add_tags_caching_methods
        send :include, Uggle::Taggable::Cache::InstanceMethods
        extend Uggle::Taggable::Cache::ClassMethods
        ...
        tag_types.map(&:to_s).each do |tag_type|
          if self.class.send("caching_#{tag_type.singularize}_list?")
            if tag_list_cache_set_on(tag_type)
              list = tag_list_cache_on(tag_type).to_a.flatten.compact.join("#{Uggle.delimiter} ")
