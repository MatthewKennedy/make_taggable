...
module Uggle
  class TagList < Array
    ...
    def clean!
      ...
      map!(&:to_s)
      map!(&:strip)
      map! { |tag| tag.mb_chars.downcase.to_s } if Uggle.force_lowercase
      map!(&:parameterize) if Uggle.force_parameterize

      Uggle.strict_case_match ? uniq! : uniq!{ |tag| tag.downcase }
      self
    end
