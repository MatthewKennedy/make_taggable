...
describe 'Acts As Taggable On' do
  ...
  describe 'Taggable Method Generation' do
    before(:each) do
      TaggableModel.tag_types = []
      TaggableModel.uggle(:tags, :languages, :skills, :needs, :offerings)
      @taggable = TaggableModel.new(name: 'Bob Jones')
    end
  ...
  describe 'Matching Contexts' do
    ...
    it 'should ensure joins to multiple taggings maintain their contexts when aliasing' do
      ...
      offer_alias = TaggableModel.connection.quote_table_name(Uggle.taggings_table)
  ...
  describe 'Tagging Contexts' do
    it 'should eliminate duplicate tagging contexts ' do
      TaggableModel.uggle(:skills, :skills)
      expect(TaggableModel.tag_types.freq[:skills]).to eq(1)
    end

    it 'should not contain embedded/nested arrays' do
      TaggableModel.uggle([:array], [:array])
  ...
  describe '@@remove_unused_tags' do
    before do
      ...
      @tagging = Uggle::Tagging.create(taggable: @taggable, tag: @tag, context: 'tags')
    end
    ...
    context 'if set to true' do
      ...
      it 'should remove unused tags after removing taggings' do
        @tagging.destroy
        expect(Uggle::Tag.find_by_name('awesome')).to be_nil
      end
    end

    context 'if set to false' do
      before do
        Uggle.remove_unused_tags = false
      end

      it 'should not remove unused tags after removing taggings' do
        @tagging.destroy
        expect(Uggle::Tag.find_by_name('awesome')).to eq(@tag)
      end
    end
  end
