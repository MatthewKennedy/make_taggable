...
describe Uggle::TagsHelper do
  ...
  it 'should handle tags with zero counts (build for empty)' do
    Uggle::Tag.create(name: 'php')
    Uggle::Tag.create(name: 'java')
    Uggle::Tag.create(name: 'c++')

    tags = {}

    @helper.tag_cloud(Uggle::Tag.all, %w(sucky awesome)) do |tag, css_class|
      tags[tag.name] = css_class
    end
