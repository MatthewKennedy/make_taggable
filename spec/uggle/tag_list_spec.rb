...
describe Uggle::TagList do
  let(:tag_list) { Uggle::TagList.new('awesome', 'radical') }
  let(:another_tag_list) { Uggle::TagList.new('awesome','crazy', 'alien') }
  ...
  describe '#+' do
    ...
    it 'should have class : Uggle::TagList' do
      new_tag_list = tag_list + another_tag_list
      expect(new_tag_list.class).to eq(Uggle::TagList)
    end
  end
  ...
  describe '#concat' do
    ...
    context 'without duplicates' do
      let(:arr) { ['crazy', 'alien'] }
      let(:another_tag_list) { Uggle::TagList.new(*arr) }
  ...
  describe 'custom parser' do
    ...
    it 'should use a the default parser if none is set as parameter' do
      allow(Uggle.default_parser).to receive(:new).and_return(parser)
      Uggle::TagList.new('cool, wicked', parse: true)
    ...
    it 'should use the custom parser passed as parameter' do
      ...
      Uggle::TagList.new('cool, wicked', parser: parser_class)
    ...
    it 'should use the parser setted as attribute' do
      ...
      tag_list = Uggle::TagList.new('example')
